/**
 * box3d-wasm ships no bundled TypeScript types. `@frsource/babylon-box3d`
 * (the parent package) already augments it for its own public API, but a
 * `declare module` augmenting an already-resolved-but-untyped module is only
 * honored for files TypeScript treats as local program roots -- not for
 * ones reached solely via a dependency's own bundled `.d.ts` (see
 * TS2665). This demo imports box3d-wasm directly (for `Box3D()` itself and
 * for the wheel joint's raw types), so it needs this same shim locally,
 * scoped to `docs/`.
 */
